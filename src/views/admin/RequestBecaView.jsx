...
const RequestBecaView = () => {
	...
	const handleBlurCapture = async (e, setValues) => {
		try {
			...
			if (axiosReponse.result == null)
				return sAlert.Info(
					"El CURP ingresado no está registrado, veritifíca que este correcto para guardarse al finalizar esta solicitud."
				);
	...
	const handleBlurCaptureByZip = async (zip) => {
		const axiosCommunity = (axios.defaults.url =
			"/https://api.gomezpalacio.gob.mx/api/cp");
		console.log(axiosCommunity);
		const axiosCommunities = await axiosCommunity.get(
			`/https://api.gomezpalacio.gob.mx/api/cp/${zip}`
		);
	...
	const onSubmit3 = async (
		...
		try {
			...
			sAlert.Customizable(
				axiosResponse.alert_text,
				axiosResponse.alert_icon
			);
			setStepFailed(-1);
			resetForm();
			resetFormData();
	...
	const validationSchema1 = Yup.object().shape({
		// id: 0,
		folio: Yup.number("solo números").required("Folio requerido"),
		tutor_full_name: Yup.string()
			.trim()
			.required("Nombre completo del tutor requerido"),
		tutor_phone: Yup.string()
			.trim()
			.min(10, "El número telefónico debe ser a 10 digitos")
			.required("Número telefonico del tutor requerido"),
	});
	const validationSchema2 = Yup.object().shape({
		...
		name: Yup.string().trim().required("Nombre(s) del alumno requerido(s)"),
		paternal_last_name: Yup.string()
			.trim()
			.required("Apellido Paterno requerido"),
		maternal_last_name: Yup.string()
			.trim()
			.required("Apellido Materno requerido"),
		birthdate: Yup.date("Fecha inválida").required(
		...
		num_ext: Yup.string().trim().required("Número exterior requerido"),
		// num_int: Yup.string().trim().required("Clave de escuela requerida"),
		disability_id: Yup.number()
			.min(1, "Ésta opción no es valida")
							...
							>
								{({
									...
									<Box
										sx={{
											height: "100%",
											display: activeStep + 1 == 1 ? "flex" : "none",
											flexDirection: "column",
											justifyContent: "space-between",
										}}
										component={"form"}
										...
										<Grid container spacing={2}>
											...
											<Grid xs={12} md={9} sx={{ mb: 3 }}>
												<TextField
													...
													// disabled={values.id == 0 ? false : true}
													error={
														errors.tutor_full_name &&
														touched.tutor_full_name
													}
													helperText={
														errors.tutor_full_name &&
														touched.tutor_full_name &&
											...
											<Grid xs={12} md={3} sx={{ mb: 3 }}>
												<TextField
													...
													helperText={
														errors.tutor_phone &&
														touched.tutor_phone &&
														showErrorInput(1, errors.tutor_phone)
													}
												/>
											</Grid>
										</Grid>
							...
							>
								{({
									...
									<Box
										sx={{
											height: "100%",
											display: activeStep + 1 == 2 ? "flex" : "none",
											flexDirection: "column",
											justifyContent: "space-between",
										}}
										component={"form"}
										...
										<Grid container spacing={2}>
											...
											<Grid xs={12} md={4} sx={{ mb: 3 }}>
												<TextField
													...
													helperText={
														errors.birthdate &&
														touched.birthdate &&
														showErrorInput(2, errors.birthdate)
													}
												/>
											</Grid>
											{/* Genero */}
											...
											<Grid xs={12} md={4} sx={{ mb: 1 }}>
												<FormControl fullWidth>
													<InputLabel id='disability_id-label'>
														Discapacidad *
													</InputLabel>
													<Select
														id='disability_id'
											...
											<Grid xs={12} md={3} sx={{ mb: 3 }}>
												<TextField
													...
													onBlur={handleBlur}
													onBlurCapture={(e) =>
														handleBlurCaptureByZip(e.target.value)
													}
													fullWidth
													disabled={values.id == 0 ? false : true}
                                       ...
                                       dataCities.map((d) => (
                                          <MenuItem key={d.value} value={d.value}>
                                             {d.code} - {d.text}
													...
													{touched.state &&
														errors.state &&
														showErrorInput(2, errors.state, true)}
												</FormControl>
											</Grid>
											{/* Ciduad */}
											<Grid xs={12} md={3} sx={{ mb: 1 }}>
                                       ...
                                       dataCities.map((d) => (
                                          <MenuItem key={d.value} value={d.value}>
                                             {d.code} - {d.text}
													...
													{touched.city &&
														errors.city &&
														showErrorInput(2, errors.city, true)}
												</FormControl>
											</Grid>
											{/* Colonia */}
											<Grid xs={12} md={3} sx={{ mb: 1 }}>
											...
											<Grid xs={12} md={8} sx={{ mb: 3 }}>
												<TextField
													...
													helperText={
														errors.street &&
														touched.street &&
														showErrorInput(2, errors.street)
													}
												/>
											</Grid>
											{/* Num Ext */}
											<Grid xs={12} md={2} sx={{ mb: 3 }}>
												<TextField
													...
													helperText={
														errors.num_ext &&
														touched.num_ext &&
														showErrorInput(2, errors.num_ext)
													}
												/>
											</Grid>
											{/* Num Int */}
											<Grid xs={12} md={2} sx={{ mb: 3 }}>
												<TextField
													...
													helperText={
														errors.num_int &&
														touched.num_int &&
														showErrorInput(2, errors.num_int)
													}
												/>
											</Grid>
											{/* <LoadingButton
							...
							>
								{({
									...
									<Box
										sx={{
											height: "100%",
											display: activeStep + 1 == 3 ? "flex" : "none",
											flexDirection: "column",
											justifyContent: "space-between",
										}}
										component={"form"}
											...
											<Grid xs={12} md={6} sx={{ mb: 3 }}>
												<TextField
													...
													helperText={
														errors.average &&
														touched.average &&
														showErrorInput(3, errors.average)
													}
												/>
											</Grid>
											{/* Comentarios */}
											<Grid xs={12} md={12} sx={{ mb: 3 }}>
												<TextField
													...
													helperText={
														errors.comments &&
														touched.comments &&
														showErrorInput(3, errors.comments)
													}
												/>
											</Grid>
										</Grid>
